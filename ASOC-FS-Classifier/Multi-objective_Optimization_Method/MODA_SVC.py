...
def sort_by_values1(list1, values):  # front   拥挤度
    sorted_list = []
    while (len(sorted_list) != len(list1)):
...
def moda_svc(name, arr):
    inputname = name
    for mm in arr:
        start1 = time.time()
        inputdata = '../' + inputname + '.csv'
        dataset = pd.read_csv(inputdata, header=None)
        path = '../ExperimentsData/Multi/' + inputname + '_MODA_' + str(int(time.time())) + '.xlsx'
        ...
        Archive_X1 = []
        Archive_X2 = []
        food_fitness = float("inf") * np.ones(2)
        food_pos = np.zeros(dim + para_size)
        enemy_fitness = -float("inf") * np.ones(2)
        enemy_pos = np.zeros(dim + para_size)
        pop = init(population_size, dim, para_size)
        Delta_pop = init(population_size, dim, para_size)
        ...
        for t in range(T):
            ...
            if index == -1:
                index = 0
            food_fitness = copy.deepcopy(Archive_F[index,0:2])
            food_pos = copy.deepcopy(Archive_X[index])
            index = RouletteWheelSelection(Archive_mem_ranks)
            if index == -1:
                index = 0
            enemy_fitness = copy.deepcopy(Archive_F[index,0:2])
            enemy_pos = copy.deepcopy(Archive_X[index])
            for i in range(pop.shape[0]):
                sumS1 = np.zeros(dim + para_size)
                Si = np.zeros(dim + para_size)
                for j in range(pop.shape[0]):
                    sumS1 = copy.deepcopy(sumS1 + (pop[j] - pop[i]))
                Si = copy.deepcopy(-sumS1)
                sumS2 = np.zeros(dim + para_size)
                Ai = np.zeros(dim + para_size)
                for j in range(pop.shape[0]):
                    sumS2 = copy.deepcopy(sumS2 + Delta_pop[j])
                Ai = copy.deepcopy(sumS2 / (pop.shape[0] - 1))
                sumS3 = np.zeros(dim + para_size)
                for j in range(pop.shape[0]):
                    sumS3 = copy.deepcopy(sumS3 + pop[j])
                Ci = copy.deepcopy(sumS3 / (pop.shape[0] - 1) - pop[i])
                Fi = copy.deepcopy(food_pos - pop[i])
                Ei = copy.deepcopy(enemy_pos + pop[i])
                Delta_pop[i] = (s * Si + a * Ai + c * Ci + f * Fi + e * Ei) + w * Delta_pop[i]
                pass
            for i in range(pop.shape[0]):
                for j in range(dim + para_size):
                    if j < dim or j == dim + 2:
                        ...
                        if Delta_pop[i][j] < -6:
                            Delta_pop[i][j] = -6
                        p = random.random()
                        v = math.fabs(Delta_pop[i][j] / math.sqrt(Delta_pop[i][j] * Delta_pop[i][j] + 1))
                        if (p < v):
                            pop[i][j] = 0 + 1 - pop[i][j]
                        pass
                    if j == dim:
                        pop[i][j] = np.clip(pop[i][j] + Delta_pop[i][j], 2 ** (-1), 2 ** (5))
                    if j == dim + 1:
                        pop[i][j] = np.clip(pop[i][j] + Delta_pop[i][j], 2 ** (-4), 2 ** (5))
